...
@author z80
@notice This is a modification of pcaversaccio's ERC721
        implementation in Snekmate. Find it here:
        https://github.com/pcaversaccio/snekmate
        These functions implement the ERC-721
        standard interface:
...
# @dev Returns the price of minting an NFT
...
@external
@payable
def __init__(name_: String[25], symbol_: String[5], base_uri_: String[80], name_eip712_: String[50], version_eip712_: String[20], price_: uint256):
    """
...
def balanceOf(owner: address) -> uint256:
    ...
    @return uint256 The 32-byte token amount owned
            by `owner`.
    """
    return self._balance_of(self.owner)
...
@payable
@external
def safe_mint(minOut: uint256 = 0):
    """
    ...
    assert msg.value >= price, "UNDERPAID"
    self._buy_milady(minOut)
...
interface Router:
    def swapExactETHForTokens(
            ...
            _to: address,
            _deadline: uint256
        ) -> DynArray[uint256, 3]: payable

@internal
def _buy_milady(minOut: uint256):
    router: Router = Router(0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F)
    path: DynArray[address, 3] = [0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2, 0x227c7DF69D3ed1ae7574A1a7685fDEd90292EB48]
    outputs: DynArray[uint256, 3] = router.swapExactETHForTokens(minOut, path, self, block.timestamp + 3600, value=self.balance)
    log TokenPurchase(outputs[0], outputs[1])
